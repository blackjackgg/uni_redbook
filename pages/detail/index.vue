	...
	<div class="public-index-wrap" v-if="dataObj">
		<scroll-view :scroll-y="true" :style="{'height':'100vh'}" @scrolltolower="lower">
			<div class="public-swiper">
				<swiper :indicator-dots="indicatorDots" :autoplay="autoplay" :interval="interval" :duration="duration" v-if="dataObj.fileType == 'image'">
				...
				<video :src="dataObj.imgs && dataObj.imgs[0].filePath" :poster="dataObj.coverPic" :loop="true" object-fit="cover"
				 v-else></video>
			</div>
			<div class="information">
				<div class="user-head">
					...
					<div class="head-r" :class="{active: dataObj.isFollow}" v-if="dataObj.showFollow" @click.stop="changeFollow(dataObj)">{{dataObj.isFollow ? '已关注' : '关注'}}</div>
				</div>
				<div class="content">
		...
		<div class="method-btn">
			<div class="comment-text" @click="changeemoji">
			...
			<div class="btn">
				...
				<div :class="{'active': dataObj.isLike}" @click.stop="changeMethod(2)">
					<img :src="dataObj.isLike ? '/static/images/likeActive.png':'/static/images/like.png'" mode="widthFix" />
					{{dataObj.like}}
				</div>
				<div class="comment">
...
<script>
	...
	export default {
		...
		methods: {
			...
			async getData() {
				let res = await this.$request.get("/api/works/detail", {
					id: this.id
				});
				if (!res.errno) {
